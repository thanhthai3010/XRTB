...
/**
 * A class for testing SMAATO Bids. Note, all tests are stubbed out. Enable the
 * test with the annotation @Test after you enable the SMAATO test code in
 * RTBServer.java.
 * 
 * @author Ben M. Faul
 *
...
public class TestSmaato {
    ...
    /**
     * You need at least one test when all the others are stubbed out or this will
     * cause a problem on the reports.
     */
    @Test
	...
	String s = Charset.defaultCharset()
		.decode(ByteBuffer.wrap(Files.readAllBytes(Paths.get("./SampleBids/smaato.json")))).toString();
	String xtime = null;
	long time = 0;
	s = Charset.defaultCharset().decode(ByteBuffer.wrap(Files.readAllBytes(Paths.get("./SampleBids/smaato.json"))))
		.toString();
	...
	long time = 0;
	String s = Charset.defaultCharset()
		.decode(ByteBuffer.wrap(Files.readAllBytes(Paths.get("./SampleBids/smaatoTXTAD.json")))).toString();
	time = System.currentTimeMillis();
	...
	long time = 0;
	String s = Charset.defaultCharset()
		.decode(ByteBuffer.wrap(Files.readAllBytes(Paths.get("./SampleBids/smaatoRICHMEDIA.json")))).toString();
	time = System.currentTimeMillis();
	s = http.sendPost("http://" + Config.testHost + "/rtb/bids/smaato?testbid=bid", s, 100000, 100000);
	time = System.currentTimeMillis() - time;
	xtime = http.getHeader("X-TIME");
