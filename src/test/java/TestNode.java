...
public class TestNode {
    ...
    /*
     * @Test public void testOs() throws Exception { List<String> parts = new
     * ArrayList(); parts.add("Android"); Node node = new
     * Node("test","device.os","MEMBER",parts); node.notPresentOk = false;
     * BufferedReader br = new BufferedReader(new FileReader("../requests"));
     * for(String line; (line = br.readLine()) != null; ) { BidRequest bidR = new
     * BidRequest(new StringBuilder(line)); boolean x = node.test(bidR); if (x ==
     * true) { JsonNode os = (JsonNode)bidR.interrogate("device.os"); if
     * (os.textValue() == null) { System.out.println("NULL"); return; }
	 ...
	 * br.setExchange( "c1x" ); assertNotNull(br); node = new
	 * Node("=","imp.0.banner.id","EQUALS","2"); b = node.test(br); // true means
	 * the constraint is satisfied. assertFalse(b);
	 * 
	 * node = new Node("=","imp.1.banner.id","EQUALS","2"); b = node.test(br); //
	 * true means the constraint is satisfied. assertTrue(b);
	 */
     ...
     * 
     * @throws Exception
     *             on file errors in configuration file.
     */
    @Test
	...
	// node = new Node("eitheror",Arrays.asList("site.domain",
	// "app.domain"),"EQUALS","smartyads");
	b = node.test(br);
	assertFalse(b);
     ...
     * 
     * @param attr
     *            List. The list of various attributes.
     * @param what
     *            String. The name you are looking for.
