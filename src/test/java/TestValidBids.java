...
public class TestValidBids {
	...
	/*
	 * BidRequest br = new BidRequest("./SampleBids/nexage.txt"); assertNotNull(br);
	 * assertNull(br.bidFloor);
	 * 
	 * br.setBidFloor(100.0); JsonNode n = (JsonNode) br.getNode("imp.0.bidfloor");
	 * assertNotNull(n); assertTrue(n.doubleValue() == 100.0);
	 */
    }
    ...
    /**
     * Test a valid bid response with no bid, the campaign doesn't match width or
     * height of the bid request
     * 
     * @throws Exception
     *             on network errors.
     */
     ...
     * Analyze crap bids
     * 
     * @Test public void testSuspect3() throws Exception { HttpPostGet http = new
     * HttpPostGet(); String bid = Charset .defaultCharset()
     * .decode(ByteBuffer.wrap(Files.readAllBytes(Paths
     * .get("./SampleBids/bad-cappture.txt")))).toString(); String s = null; long
     * time = 0; String xtime = null;
     * 
     * try { try { time = System.currentTimeMillis(); s = http.sendPost("http://" +
     * Config.testHost + "/rtb/bids/nexage", bid,300000,300000); time =
     * System.currentTimeMillis() - time; xtime = http.getHeader("X-TIME"); } catch
     * (Exception error) { fail("Can't connect to test host: " + Config.testHost); }
     * 
