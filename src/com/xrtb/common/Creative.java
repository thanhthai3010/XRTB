...
public class Creative {
	...
	/*
	 * if (forwardurl != null) { if (forwardurl.contains("<script") ||
	 * forwardurl.contains("<SCRIPT")) { if (forwardurl.contains("\"") &&
	 * (forwardurl.contains("\\\"") == false)) { forwardurl =
	 * forwardurl.replaceAll("\"", "\\\\\""); } } }
	 */
     ...
     * 
     * @param impid
     *            String. The impression id to use for this creative. This is merely
     *            a databse key you can use to find bids and wins for this id.
     */
     ...
     * @param errorString
     *            StringBuilder. The string to hold any campaign failure messages
     * @return boolean. Returns true of this campaign matches the bid request, ie
     *         eligible to bid
     */
    public SelectedCreative process(BidRequest br, Map<String, String> capSpecs, String adId, StringBuilder errorString,
	    Probe probe) {
    ...
    public SelectedCreative xproc(BidRequest br, String adId, Impression imp, Map<String, String> capSpecs,
	    StringBuilder errorString, Probe probe) {
	...
	if (imp.deals != null) {
		...
		/*
		 * newDeals = new ArrayList<Deal>(deals); newDeals.retainAll(imp.deals); if
		 * (newDeals.size() != 0) { dealId = newDeals.get(0).id; xprice =
		 * newDeals.get(0).price; Deal brDeal = imp.getDeal(dealId);
		 * 
		 * if (brDeal == null && price == 0) { probe.process(br.getExchange(), adId,
		 * impid, Probe.PRIVATE_AUCTION_LIMITED); if (errorString != null)
		 * errorString.append(Probe.NO_WINNING_DEAL_FOUND); return null; }
		 * 
		 * imp.bidFloor = new Double(brDeal.price); } else if (price == 0 ||
		 * imp.privateAuction == 1) { probe.process(br.getExchange(), adId, impid,
		 * Probe.NO_APPLIC_DEAL); if (errorString != null)
		 * errorString.append(Probe.NO_APPLIC_DEAL); return null; }
		 */
	    } else {
     ...
     * 
     * @param br
     *            BidRequest. The bid request to query.
     * @param capSpecs.
     *            The current cap spec.
