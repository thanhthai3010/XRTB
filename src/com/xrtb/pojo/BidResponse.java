...
public class BidResponse {
    ...
    /**
     * The time it took to build the request in milliseconds (campaign processor
     * time)
     */
    public int xtime;
    /**
     * The region field, may be added by crosstalk, but if not using crosstalk, will
     * be null
     */
    ...
    /**
     * type of ad, video, banner, native. Was 'type', elastic search doesn;t like
     * that
     */
     ...
     * @param camp
     *            . Campaign - the campaign that will be used to form the response.
     * @param oidStr
     *            . String - the unique id for this response.
     */
    public BidResponse(BidRequest br, Impression imp, Campaign camp, Creative creat, String oidStr, double price,
     ...
     *            BidRequest used
     * @param multi
     *            List. The multiple creatives that bid.
     * @param xtime
     *            int. The time to process.
     * @throws Exception
     */
    ...
    /**
     * While we can't uuencode the adm for smaato (pesky XML tags, we have to change
     * & to &amp;
     * 
     * @param sb
	...
	while (i < sb.length()) {
	    ...
	    if (i == -1)
		return;
	    if (!(sb.charAt(i + 1) == 'a' && sb.charAt(i + 2) == 'm' && sb.charAt(i + 3) == 'p'
		    && sb.charAt(i + 4) == ';')) {
    ...
    /**
     * Creates a template for the smaato exchange, which has an XML format for the
     * ADM
     */
    ...
    /**
     * Return the adm as a string. If video, use the encoded one in the creative,
     * otherwise jusr return
     * 
    ...
    /**
     * Return whether this is a no bid. For openRTB it always returns false because
     * we won't make this object when http response code is 204. Adx always returns
     * 200.
     * 
     * @return
     */
