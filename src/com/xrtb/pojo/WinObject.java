...
public class WinObject {
    ...
    /**
     * The worker method for converting a WIN http target into a win notification in
     * the bidder.
     * 
     * @param target
     *            String. The HTTP url that makes up the win notification from the
     *            exchange.
	...
	/*
	 * This is synthetic, because in reality, adx has no win notification, this is a
	 * fake pixel fire that does the work
	 */
	...
	try {
	    ...
	    logger.error(
		    "Error: exchange {} did not pass a proper {AUCTION_PRICE} substitution ont the WIN, win price is undeterimed: {}",
		    pubId, price);
	}
    }
}
