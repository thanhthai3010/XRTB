...
public class CampaignSelector {
	...
	if (!Configuration.multibid)
	    winner = br.buildNewBidResponse(select.getImpression(), select.getCampaign(), select.getCreative(),
		    select.getPrice(), select.getDealId(), (int) xtime);
	else {
    ...
    /**
     * Creates a forced bid response on the specified bid request. owner, campaign
     * and creative.
     * 
     * @param br
