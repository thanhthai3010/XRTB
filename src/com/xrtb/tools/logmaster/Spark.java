...
public class Spark implements Runnable {
     ...
     * 
     * @param args
     *            . The arguments: -redis host:port Set the redis host/port, default
     *            localhost:6379 -clicks clicklogchannel Set the click log channel
     *            to listen to, default is clicks -bids Set the bids channel to
     *            listen to, default is bids -nobids Set the nobids channel to
     *            listen to, default is nobids -wins Set the wins channel to listen
     *            to, default is wins -requests Set the requests channel to listen
     *            to, default is requests -logdir Set the logdir, default is ./logs
	...
	if (args.length > 0) {
		...
		case "-h":
		    System.out
			    .println("-aero  host:port   [Set the Aerospike host and port, default localhost:6379  ]");
		    System.out
			    .println("-logdir dirname    [Where to place the logs, default is ./logs               ]");
		    System.out.println(
			    "-purge               [Delete the log records already produced, default no purge]");
		    System.out.println(
			    "-interval            [Set the accounting interval, default is 60000 (60 seconds)]");
		    i++;
		    break;
		case "-clicks":
    ...
    /**
     * Periodic processor. This writes the summary account/creative records. Which
     * are summaries from the last period (which is 1 minute).
     */
    public void run() {
	while (true) {
	    try {
