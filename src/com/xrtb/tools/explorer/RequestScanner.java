...
/**
 * A quick and dirty requst map reduce scanner, for use with archived JSON
 * records of the bid requests
 * 
 * @author Ben M. Faul
 *
 */
public class RequestScanner {
	...
	System.out.println("Count = " + count + "\nInstl = " + instlCount + ", (" + (instlCount / count * 100.0) + ")"
		+ "\nBid Floor = " + bidFloorCount + ", (" + (bidFloorCount / count * 100.0) + ")"
		+ "\nAvg Bid Floor = " + (bidFloorValue / bidFloorCount) + "\n\nBanners = " + bannerCount + ", ("
		+ (bannerCount / count * 100) + ")"

		+ "\nVideo = " + videoCount + ", (" + (videoCount / count * 100) + ")"

		+ "\nNative = " + nativeCount + ", (" + (nativeCount / count * 100) + ")"

		+ "\n\nAvg Banner Bid Floor = " + bannerFloorValue / bannerCount + "\nOur Size Banners = " + ourSize
		+ ", (" + (ourSize / count * 100) + ")" + "\nAvg Our Size Bid Floor = " + (ourSizeBidFloor / ourSize) +

		"\nMimes of Our Size = " + mimeCount + ", (" + (mimeCount / ourSize * 100) + ")"
		+ "\nAPIs of Our Size = " + apiCount + ", (" + (apiCount / ourSize * 100) + ")" +

		"\nAvg Non API Bid Floor = " + (nonApiFloorValue / (ourSize - apiCount)) + "\nAvg API Bid Floor = "
		+ (apiFloorValue / (apiCount)) +

		"\nGeo = " + geoCount + ", (" + (geoCount / count * 100) + ")" + "\nOur Geo = "
		+ (ourGeoCount / ourSize * 100) +

		"\n");

	List<Quartet> qlist = reduceQuartet(allIabs, (int) count);
	System.out.println("Not categorized: " + noCat + " (" + (noCat / count * 100) + ")");
	...
	// tups.forEach((q) -> System.out.printf("%s, %d,
	// (%.3f%%)\n",q.site,q.count,q.percent));
