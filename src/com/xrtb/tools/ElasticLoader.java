...
public class ElasticLoader {
	...
	while (i < args.length) {
		...
		System.out.println(
			"-host <host:port> [Set the host:port for the bidder, default localhost:8080                        ]");
		System.out.println(
			"-geo filename     [Sets the geo zip code file, default data/unique_geo_zipcodes.txt                ]");
		System.out.println(
			"-ssp \"ssp1, ...\"[Sets the ssp names to use.                                                      ]");
		System.out.println(
			"-win <n>          [Set the percentage to win, default 80                                           ]");
		System.out.println(
			"-pixel <n>        [Set the percentage to pixel fires, default 90                                   ]");
		System.out.println(
			"-click <n>        [Set the percentage to clicks, default 3                                         ]");
		System.out.println(
			"-silent           [Don't show the transactions, default is to show them.                           ]");
		System.out.println(
			"-count <n>        [Number of times to execute, default 1000000                                     ]");
		System.out.println(
			"-forever          [loop on end of file on the file                                                 ]");
	    case "-file":
		fileName = args[i + 1];
		i += 2;
		break;
