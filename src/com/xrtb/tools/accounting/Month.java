...
public class Month {
	...
	if (!hourly) {
		...
		System.out.println(
			"Date             Bids             Wins        Bid Price        Win Price           Pixels           Clicks");
	}
	...
	if (hourly) {
		...
		System.out.println(
			"                 Bids             Wins        Bid Price        Win Price           Pixels           Clicks");
	}

	if (Process.nz && bids == 0) {

	} else {
	    String result = String.format("TOTAL%16d %16d %16.4f %16.4f %16d %16d ", bids, wins, bidP, winP, pixels,
		    clicks);
