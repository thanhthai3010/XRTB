...
public class Process {
     ...
     * 
     * @param args
     *            String[]. The accounts processing. Arguments: -source <filename>
     *            Set the input filename. Default is logs/accouunting -year <year>
     *            Set the year. Default is this year -startMonth <month> Set the
     *            month to start the report. Default is current momth. Jan = 1
     *            -endMonth <month> Set the month to end the report. Default is
     *            current month. Jan = 1 -hourly Do hourly instead of daily reports.
     *            Defaults to not hourly (by day) -day <day of month> Set the days
     *            to do the report on (within months). Default is today. Can specify
     *            multiple days like -day 1 -day 2 -day 3 Writes report to stdout.
     *            Defaults to -startMonth <now> -stopMonth <now> -year <now> -source
     *            logs/accounting
     * @throws Exception
     */
	...
	for (File f : listOfFiles) {
		...
		/*
		 * fr = new FileReader(f); bufr = new BufferedReader(fr); while ((content =
		 * bufr.readLine()) != null) { Object x = null; if
		 * (f.getName().startsWith("bids")) x = mapper.readValue(content,
		 * BidResponse.class); else if (f.getName().startsWith("wins")) x =
		 * mapper.readValue(content, WinObject.class); else if
		 * (f.getName().startsWith("request")) x = mapper.readValue(content, Map.class);
		 * 
	    ...
	    /*
	     * while (procs.size() > 0) { for (FileProcessor p : procs) { if (p.done) {
	     * System.out.println("Completed: " + p.fileName); procs.remove(p); break; } } }
	     */
	}
