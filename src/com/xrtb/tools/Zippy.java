...
/**
 * Creates the HTTP handler for the Minimal server. Whatever it gets, it dumps
 * to stdout
 * 
 * @author Ben M. Faul.
 *
...
public class Zippy implements Runnable {
     ...
     * 
     * @param args
     *            . String[]. Args[0] contains the name of the users file, if not,
     *            presume "users.json"
     * @throws Exception
	...
	if (args.length != 0) {
		...
		case "-h":
		    ...
		    System.out.println(
			    "-p portnum      [Port number to listen to                                           ]");
		    System.out.println(
			    "-b dir          [Base directory when -s is used                                     ]");
		    System.out.println(
			    "-s              [Save to file (based off of base directory / uri), default is stdout]");
		    return;
		case "-p":
