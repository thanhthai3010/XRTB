...
public class WebPatch {
	...
	while (i < args.length) {
	    ...
	    case "-h":
		System.out
			.println("-www <directory>          [The parent directory in front of the www, default is ./]");
		...
		System.out.println(
			"-aero <aero-host>       [The hostname of where redis lives (do not specify the port]");
		System.out.println("-webdis <redis-host:port> [The hostname of where webdis lives]");
		System.out.println(
			"-service homedir         [The Login home directoy (for /home/ben use -service ben, default is ubuntu)]");
		System.out.println(
			"-demo true | false        [Whether to set the demo mode in login.html and admin.html]");
		System.exit(1);
	    case "-www":
		fix = args[++i];
		i++;
