...
public class Stroer extends BidRequest {
	...
	if (avr == null || avn == null)
	    throw new Exception(
		    camp.adId + "/" + creat.impid + " is missing required avn or avr extension for Stroer SSP");
	...
	for (int i = 0; i < multi.size(); i++) {
		...
		throw new Exception(
			x.getCampaign().adId + "/" + c.impid + " is missing required extensions for Stroer SSP");

	    avr = c.extensions.get("avr");
	    avn = c.extensions.get("avn");

	    if (avr == null || avn == null)
		throw new Exception(x.getCampaign().adId + "/" + c.impid
			+ " is missing required avn or avr extension for Stroer SSP");
