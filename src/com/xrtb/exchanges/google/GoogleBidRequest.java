...
public class GoogleBidRequest extends BidRequest {
     ...
     * 
     * @param in
     *            InputStream. The content of the POST
     * @throws Exception
     *             on protobuf, json or I/O errors.
     */
     ...
     * 
     * @param br
     *            com.google.openrtb.OpenRtb.BidRequest. The bid request in
     *            protobuf.
     * @throws Exception
     *             on JSON errors.
     */
     ...
     * 
     * @param node
     *            ObjectNode. The parent JSON node
     * @param g
     *            Geo. The protobuf geo encoding.
     */
	...
	if (internal.hasSite()) {
		...
		if (c.hasContentrating())
		    content.put("contentrating", content);
		if (c.hasContext22())
     ...
     * 
     * @param pmp
     *            Pmp. Private market place on protobuf.
     * @param impx
     *            ObjectNode. The JSON equivalent.
     */
     ...
     * 
     * @param array
     *            ArrayNode. Holds the impressions.
     * @param b
     *            Banner. The protobuf banner.
	...
	if (b.hasH())
	    node.put("h", b.getH());
	if (b.hasHmax())
	    node.put("hmax", b.getHmax());
	if (b.hasHmin())
	    node.put("hmin", b.getHmin());
	if (b.hasPos())
	...
	if (b.hasW())
	    node.put("w", b.getW());
	if (b.hasWmax())
	    node.put("wmax", b.getWmax());
	if (b.hasWmin())
	    node.put("wmin", b.getWmin());
	...
	if (v.hasProtocol())
	    video.put("protocol", v.getProtocol().getNumber());
     ...
     * 
     * @param node
     *            ArrayNode. The node we will add to.
     * @param list
     *            List. A list of creative attributes.
     ...
     *            ArrayNode. The node we will add to.
     * @param list
     *            List. A list of banner ad types.
     * @return ArrayNode. The node we passed in.
     */
     ...
     *            ArrayNode. The node we will add to.
     * @param list
     *            List. A list of protocol numbers.
     * @return ArrayNode. The node we passed in.
     */
     ...
     *            ArrayNode. The node we will add to.
     * @param list
     *            List. A list of protocol strings.
     * @return ArrayNode. The node we passed in.
     */
     ...
     * 
     * @param node
     *            ArrayNode. The list to add to.
     * @param list
     *            List. The list of companiontypes.
     ...
     * 
     * @param creat
     *            Creative. The creative in question.
     * @param errorString
     *            StringBuilder. The error handling string. Add your error here if
     *            not null.
