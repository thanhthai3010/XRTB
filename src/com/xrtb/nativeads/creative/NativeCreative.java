...
public class NativeCreative {
    ...
    /**
     * The data asset of this creative. Transient because in the campaign it is
     * defined with all the assets in an array, but we will pull out img, video,
     * link, and title so we can quickly find them later. Only data assets are in
     * the array after it is encoded.
     */
    transient public Map<Integer, Entity> dataMap = new HashMap();
    /** The img asset of this creative */
    transient public Asset img;
    /**
     * The video asset of this creative. Transient because in the campaign it is
     * defined with the assets array, but we will pull it out when the campaign is
     * created so we can quickly find it later
     */
    transient public Asset video;
    /**
     * The title asset of this creative. Transient because in the campaign it is
     * defined with the assets array, but we will pull it out when the campaign is
     * created so we can quickly find it later
     */
    ...
    /**
     * Pulls the image, video, link, and title assets out of the array and assigns
     * them to objects. The data assets are assigned to a hashmap by type for fast
     * lookup O(1) in the selection of creatives.
     */
    public void encode() {
	for (Asset a : assets) {
	    if (a.title != null) {
    ...
    /**
     * Create the encoded native ADM from the assets of the creative matches with
     * the assets of the bid request.
     * 
     * @param br
     *            BidRequest. The bid request of this transaction
	...
	for (int i = 0; i < assets.size(); i++) {
	    ...
	    /**
	     * If -1 is returned, then the creative has a native ad component that the bid
	     * request didn't ask for. We presume this is ok and will bid, since the
	     * creative did have all the other required pieces.
	     */
	    if (i != -1) {
