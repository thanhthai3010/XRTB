...
public class Google {
    ...
    public void request() {
	BidRequest request = BidRequest.newBuilder().setId("1").addBcat("IAB1")
		.addImp(Imp.newBuilder().setId("1").setBidfloor(4000).setVideo(Video.newBuilder()
			.setLinearity(VideoLinearity.LINEAR).addProtocols(Protocol.VAST_3_0).setW(640).setH(480)))
		.build();
    }
}
